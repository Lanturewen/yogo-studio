...
(async()=>{
 ...
 if(!['play','auto','stop','quit','status','preset'].includes(cmd)||cmd==='preset'&&!['check','install','restore'].includes(arg))throw new Error('Usage: cli.cjs list | play <id> | auto | stop | quit | status | preset <check|install|restore> | doctor');
 ...
 const route={play:'/state',auto:'/auto',stop:'/stop',quit:'/shutdown',preset:'/keyboard-preset'}[cmd];
 const body=cmd==='play'?{state:arg}:cmd==='preset'?{action:arg}:{};
 ...
 const s=await response.json();if(!response.ok||cmd!=='preset'&&s.error)throw new Error(s.error||'Request failed');console.log(JSON.stringify(cmd==='preset'?s.keyboardPreset:s));
