...
- 微信输入法语音快捷键：Mac `右 Option`，Windows `Ctrl+F5`，需要用户在输入法设置和官方键盘插件中配置；YOGO Studio 不修改底部自定义键。“监听待命”和演示波形均不等于真实收音。
- Mac 系统辅助功能、输入监控、麦克风授权由用户完成。已授权仍失效时检查应用路径与旧签名条目，指导正常移除旧条目并重新添加；不要修改 TCC 数据库或关闭系统保护。
- 在官方键盘插件中将波轮左转设为 `-`、右转设为 `=`、按下设为 `Enter`，两颗底部自定义键也在官方插件中设置。YOGO Studio 不再提供板载映射的检查、写入或恢复接口；不要尝试用旧版 CLI 写入键位。旧版备份留在数据目录，不自动删除。
...
- 硬件：真实试播后恢复自动跟随；在官方插件完成映射后，波轮左右在 Codex 输入框实际调档，按下波轮的 Enter 正常。
